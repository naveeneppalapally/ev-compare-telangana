  ...
  return (
    ...
    >
      ...
      <div className="relative w-full max-w-5xl bg-white border border-stone-200 rounded-3xl shadow-2xl overflow-hidden z-10 my-auto flex flex-col max-h-[92vh]">
        ...
        <div className="flex overflow-x-auto border-b border-stone-200 bg-stone-100/70 p-2 gap-2 scrollbar-none">
          {pillars.map(p => {
        ...
        <div className="grid grid-cols-1 md:grid-cols-12 flex-1 overflow-y-auto">
                  ...
                  <div className="flex items-center justify-between">
                    <span
                      className={`text-xs font-bold ${
                        isSelected ? 'text-stone-900 font-extrabold' : 'text-stone-800'
                      }`}
                    >
                      {t.title}
                    </span>
          ...
          <div className="md:col-span-8 p-5 sm:p-6 space-y-5 overflow-y-auto bg-white">
            ...
            <div className="p-4 rounded-2xl bg-stone-50 border border-stone-200 text-xs sm:text-sm text-stone-800 leading-relaxed font-medium">
              💡 <span className="font-bold text-stone-900">Quick Summary: </span>
            ...
            <div className="p-4 rounded-2xl bg-stone-100 border border-stone-200 flex items-start gap-3">
              <Flame className="w-4 h-4 text-stone-700 shrink-0 mt-0.5" />
            ...
            <div className="space-y-2">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
                {currentTopic.bulletPoints.map((point, i) => (
                  <div
                    key={i}
                    className="flex items-center gap-2 p-2.5 rounded-xl bg-stone-50 border border-stone-200 text-xs text-stone-800 font-medium"
                  >
            ...
            {currentTopic.exampleVehicleModelIds.length > 0 && (
              <div className="space-y-3 pt-2 border-t border-stone-200">
                <div className="flex items-center justify-between">
                  <h3 className="text-[10px] font-bold uppercase tracking-wider text-stone-500 flex items-center gap-1.5">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
                  {currentTopic.exampleVehicleModelIds.map(modelId => {
                    ...
                    return (
                      <div
                        key={modelId}
                        className="flex items-center justify-between p-3 rounded-2xl bg-white border border-stone-200 hover:border-stone-300 transition shadow-2xs"
                      >
                        <div className="flex items-center gap-2.5">
                          <img
                            src={`/images/vehicles/${vehicle.id}.jpg`}
                            alt={vehicle.name}
                            className="w-12 h-9 object-contain rounded-md bg-stone-50 p-0.5 border border-stone-200"
                            onError={(e) => {
                        ...
                        {onSelectVehicle && (
                          <button
                            ...
                            className="p-1.5 rounded-full bg-stone-100 hover:bg-stone-200 text-stone-800 transition cursor-pointer"
...
export default EVTechExplorerModal;
